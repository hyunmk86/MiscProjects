...
def getWordScore(word, n):
    ...
    returns: int >= 0
    """
    localvalue = SCRABBLE_LETTER_VALUES
    counter = 0
    for alpha in word:
        counter += localvalue[alpha]
...
def updateHand(hand, word):
    ...
    updated = {key:newdict[key] - worddict.get(key,0) for key in newdict.keys() }
    return updated
...
# word is
    ...
    for x in word:
        if newdict.get(x,0) == 0:
            return False
        if worddict[x] > newdict[x]:
            return False
    return True
